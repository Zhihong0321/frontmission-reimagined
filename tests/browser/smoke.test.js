...
const expectedWorldHeader = '// GENERATED by make-world.js from repository-local data/ — do not hand-edit.';
const expectedWorldSha256 = '26063b3e3680a190b79843604107977331922c77397dfe2a1bf23a5a3160712a';
...
test('Keeper chart boots, paints, opens ops, and crosses the browser bridge', async ({ page, request }) => {
  ...
  try {
    const provenanceResponse = await request.get('/chart/world.js');
    expect(provenanceResponse.status(), evidence(logs)).toBe(200);
    const provenanceBytes = await provenanceResponse.body();
    const provenanceText = provenanceBytes.toString('utf8');
    expect(provenanceText.split('\n', 1)[0], evidence(logs)).toBe(expectedWorldHeader);
    expect(createHash('sha256').update(provenanceBytes).digest('hex'), evidence(logs)).toBe(expectedWorldSha256);

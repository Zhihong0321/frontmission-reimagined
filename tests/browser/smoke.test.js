...
test('Keeper chart boots, paints, opens ops, and crosses the browser bridge', async ({ page, request }) => {
  ...
  try {
    // The `?view=lon,lat,zoom` deep link is the frontend's own boot-time prewarm path:
    // when zoom exceeds ZOOM_TILE_AT it calls startTileWorker()/wantTile() synchronously
    // during boot(), before the base map even finishes painting. Praha's real coordinates
    // at zoom 4 land the prewarm window inside real, tiled terrain, so this exercises the
    // production trigger instead of a synthetic wheel gesture driving cam.z past the
    // threshold (which reaches the chart's maximum zoom and a pre-existing canvas `arc`
    // edge case unrelated to the tile worker itself).
    const response = await page.goto('/chart/?view=14.4,50.1,4', { waitUntil: 'domcontentloaded' });
    ...
    // The deep link already drove startTileWorker()/wantTile() during boot(), before
    // navigation even settled. The `page.on('worker', ...)` listener above was attached
    // before `goto`, so it cannot have missed the spawn; poll the array it filled rather
    // than racing a fresh one-shot `waitForEvent`, which would already be too late.
    await expect.poll(() => workers.some((worker) => worker.url().includes('chart-tiles-worker.js')), { timeout: 15000 }).toBe(true);
    const tileWorker = workers.find((worker) => worker.url().includes('chart-tiles-worker.js'));
